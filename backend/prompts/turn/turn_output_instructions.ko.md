...
매 턴 플레이어에게 기본 생존 보상을 제공하여 Signal 완전 고갈을 방지합니다. 이를 통해 데모 루프(15~25턴) 동안 잔액이 0에 도달하지 않는 플레이 경험을 보장합니다.

1. **기본 보상 (Base Reward, 매 턴 적용)**:
   - 매 턴 플레이어는 **1~3 Signal**을 기본 생존 보상으로 획득합니다.
   - 퀘스트 완료나 earn_* 카드 선택이 없더라도, **모든 턴에서** `gains.signal >= 1`로 설정합니다.
   - 위험하거나 비용이 큰 행동을 할 때는 더 높은 기본 보상(2~3)을 지급합니다.
   - `balance_after.signal < 10`이면 기본 보상을 **3**으로 올립니다.

2. **퀘스트 보상 가이드라인**:
   - 서브 목표 완료: `reward_signal`은 **5~8** Signal이 적절합니다.
   - 주 목표 완료: `reward_signal`은 **10~15** Signal이 적절합니다.
   - 퀘스트 보상 지급 시, 기본 보상 위에 **`gains.signal`에 합산**합니다.
   - 예: 기본 보상 2 + 퀘스트 보상 8 → `gains.signal = 10`.

3. **탐색 보상 (earn_* 카드)**:
   - 플레이어가 `earn_*` 카드를 선택하여 성공하면 **3~8 Signal**을 지급합니다.
   - `gains.signal`에 적절히 반영합니다 (기본 보상 + 탐색 보상).
   - 부분 성공이더라도 최소 **2 Signal**을 지급합니다.

4. **밸런스 목표**:
   - 플레이어의 Signal 잔액을 항상 **15 이상**으로 유지하는 것을 목표로 합니다.
   - `balance_after.signal < 10`이면 반드시: 기본 보상을 3으로 올리고, 액션 덱에 `earn_*` 카드를 최소 1장 포함합니다.
   - 플레이어의 Signal이 0이면서 획득 방법이 전혀 없는 상황을 만들지 마세요.
