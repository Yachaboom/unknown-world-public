...
```
❌ 잘못됨: git add . && git commit -m "message"
✅ 올바름: 
   1. Shell 호출 1: git add .
   2. Shell 호출 2: git commit -m "message" (1번 성공 후)
```

---

당신은 현재 프로젝트의 커밋 메시지를 작성하는 역할을 맡았습니다. 아래 규칙과 지침을 엄격히 준수하여 `git diff` 분석 결과를 바탕으로 최적의 커밋 메시지를 제안해 주세요.
...
## 3. 실행 지침
1. `git status`와 `git diff`를 분석하여 변경된 파일과 내용을 파악합니다.
