...
```
❌ 잘못됨: cd frontend && npm install && npm run build
✅ 올바름: 
   1. Shell 호출 1: npm install (working_directory: "frontend")
   2. Shell 호출 2: npm run build (working_directory: "frontend", 1번 성공 후)
```
...
- ⚠️ 모든 문서 작성이 완료될 때까지 에이전트 동작을 멈추지 말고 끝까지 수행해야 합니다.
...
- **Executive Summary**
  ...
  - **생성된 모든 리팩토링 유닛에 대한 우선순위 목록** (ID, 심각도, 기대효과 요약 포함)
- **비유닛 작업 커밋 메시지 추천**
  - `@.gemini/rules/commit-rules.md`를 참고하여, 리팩토링 제안서 생성 및 관련 문서 업데이트 등 비유닛 작업(docs, chore 등)에 대한 커밋 메시지를 제안합니다.
