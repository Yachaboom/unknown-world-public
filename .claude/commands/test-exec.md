...
- 테스트 실행은 단계별로 별도 Shell 호출로 순차 실행

```
❌ 잘못됨: npm run lint && npm run test:unit && npm run test:integration
✅ 올바름: 
   1. Shell 호출 1: npm run lint
   2. Shell 호출 2: npm run test:unit (1번 성공 후)
   3. Shell 호출 3: npm run test:integration (2번 성공 후)
```
