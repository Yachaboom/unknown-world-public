...
```
❌ 잘못됨: mkdir -p dir && cd dir && touch file.txt
✅ 올바름: 
   1. Shell 호출 1: mkdir -p dir
   2. Shell 호출 2: touch file.txt (working_directory: "dir")
```
...
1. **중단 없는 수행**: 모든 문서 업데이트 및 관련 작업이 완료될 때까지 에이전트 동작을 멈추지 말고 끝까지 수행합니다.
2. **영향도 분석**: 변경사항이 미치는 범위 파악 (로드맵, 유닛 계획서, 관련 문서)
3. **품질 일관성 유지**: 기존 문서의 상세 수준과 형식을 유지
4. **일관성 유지**: 모든 문서 간 정보 동기화 보장
5. **의존성 재검증**: 변경으로 인한 의존성 충돌 해결
6. **진행률 재계산**: 완료/진행중인 작업 고려한 현실적 일정 조정
7. **리스크 재평가**: 새로운 위험 요소 식별 및 대응 방안 수립
...
### 📝 비유닛 작업 커밋 메시지 추천
- `@.gemini/rules/commit-rules.md`를 참고하여 로드맵 업데이트, PRD 수정 등 비유닛 작업에 대한 커밋 메시지를 제안합니다.
