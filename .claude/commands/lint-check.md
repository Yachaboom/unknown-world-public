...
- 포맷팅, 린트, 타입 체크는 각각 별도 Shell 호출로 순차 실행

```
❌ 잘못됨: npm run format && npm run lint && npm run typecheck
✅ 올바름: 
   1. Shell 호출 1: npm run format
   2. Shell 호출 2: npm run lint (1번 성공 후)
   3. Shell 호출 3: npm run typecheck (2번 성공 후)
```
