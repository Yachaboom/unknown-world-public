...
## 2026-02-10 - scenario-enhancement (U-122 데모 영상 시나리오 보강)

### 변경 요약

**U-122[Mmp] 데모 영상 시나리오를 5개 핵심 시나리오 중심으로 구체화**. 기존 개괄적 Core Gameplay 구조를 (1) 자유 텍스트 입력, (2) 사진 업로드→아이템 획득(`test.png`), (3) 정밀조사→핫스팟 클릭(Gemini Agent Vision), (4) 정밀조사→아이템 DnD, (5) 엔딩 세션 보고서의 5개 시나리오로 세분화. FHD(1920x1080) 촬영 명시, 자막 14개로 보강, Playwright 스크립트 시나리오별 재구성.

### 영향받은 문서

- ✏️ `vibe/unit-plans/U-122[Mmp].md`: 완료 기준(5개 시나리오)/1단계 시나리오 스크립트(시간 배분 재설계)/자막 SRT(14개)/2단계 FHD 강조/3단계 Playwright 스크립트(시나리오별)/해상도 주의사항 — 전면 보강
...
- U-122[Mmp]: 시나리오 구체화 (개괄적 Core Gameplay → 5개 핵심 시나리오 기반), FHD 해상도 명시, `vibe/ref/sample/test.png` 테스트 이미지 참조 추가, 자막 12개→14개, Playwright 코드 시나리오별 세분화

### 의존성 변경

- 변경 없음 (U-122의 의존성 U-119, U-120 유지)

### 진행률 변화

- **MVP**: 138/138 (100%) → 138/138 (100%) — 변경 없음
- **MMP**: 0/5 (0%) → 0/5 (0%) — 변경 없음
- **전체**: 138/142 (97.2%) → 138/142 (97.2%) — 변경 없음

### 품질 검증 결과

- 품질 기준 문서: U-119[Mmp], U-120[Mmp]
- 수정된 유닛 계획서: 1개 (U-122 시나리오 보강), 상세도 대폭 향상 확인
- 필수 섹션 누락 없음, 기존 문서 대비 정보량 향상 (5개 시나리오 구체화, SRT 14개 자막, Playwright 시나리오별 코드)

### 주의사항

- 시나리오 시간 배분 합계: 20+30+25+30+30+20+17+8 = **180초 (3분 정확히)**
- Scenario 2 테스트 이미지: `vibe/ref/sample/test.png` 사전 준비 필요
- Scenario 3/4의 "정밀조사(Investigate)" Action 카드가 시연 환경에서 노출되는지 사전 확인 필요
- Scenario 5(엔딩 세션 보고서)는 데모용 시나리오 단축으로 빠르게 도달해야 함 → U-025(엔딩 리포트) 기능과 연계

---

## 2026-02-10 - rescope/public-repo-integration + backlog-restore (공개 레포 준비 통합 + 백로그 복원)

### 변경 요약

**U-138을 "공개 저장소 준비 통합(레포 이전+문서 영문화+파일 정리)"으로 확대 재편**. 공개 저장소(`Yachaboom/unknown-world-public`)로 origin 변경, vibe/ 중 prd·tech-stack·architecture만 영문화→`docs/` 이동, 나머지 삭제 계획을 U-138에 통합. **U-138을 U-120 전에 배치**하여 정리된 공개 레포 기반으로 배포. 이전 커밋에서 누락된 백로그(CP-MVP-03, MMP M5 전체) 복원.

### 영향받은 문서

- ✏️ `vibe/roadmap.md`: 백로그(CP-MVP-03 + MMP M5 6개 + M6 skip) 복원, M5 순서 재편(U-138→U-120), 빠른 실행/크리티컬 패스 갱신, 제출 요건 매핑 설명 갱신, 메모 추가
- ✏️ `vibe/unit-plans/U-138[Mmp].md`: "공개 저장소 준비 통합"으로 전면 재작성 — 공개 레포 URL 확정, 문서 영문화+docs/ 구성, origin 전환, 비밀감사, 60분으로 상향
- ✏️ `vibe/unit-plans/U-120[Mmp].md`: 의존성에 U-138 추가, 배경에 공개 레포 기반 배포 명시
- ✏️ `vibe/changelog.md`: 본 이력 추가
...
- U-138[Mmp]: "공개 저장소 이전 + 불필요 파일 정리" → **"공개 저장소 준비 통합 — 레포 이전 + 문서 영문화 + 파일 정리"** — vibe 핵심 문서 3개 영문화→docs/ 이동, origin 전환 통합, 예상 소요 45분→60분
- U-120[Mmp]: Depends 변경 (이전: None → 이후: U-138) — 공개 레포 정리 완료 후 배포

**복원 (누락된 백로그)**:

- CP-MVP-03: 10분 데모 루프 체크포인트 (⏸️ 대기) — 이전 커밋에서 누락됨
- MMP M5 전체: U-119, U-138, U-120, U-121, U-122, CP-SUB-01 (모두 ⏸️ 대기) — 이전 커밋에서 누락됨
- M6 skip 노트 — 이전 커밋에서 누락됨

### 의존성 변경

- U-120[Mmp]: Depends 변경 (이전: None → 이후: U-138) — 공개 레포 기반 배포
- M5 실행 순서: ~~U-119+U-120+U-138 병렬~~ → **U-119+U-138 병렬 → U-120 → U-121 → U-122**

### 진행률 변화

- **MVP**: 137/137 (100%) → 137/137 (100%) — 변경 없음
- **MMP**: 0/5 (0%) → 0/5 (0%) — 변경 없음
- **전체**: 137/142 (96.5%) → 137/142 (96.5%) — 변경 없음

### 품질 검증 결과

- 품질 기준 문서: U-006[Mvp], U-025[Mvp]
- 수정된 유닛 계획서: 2개 (U-138 전면 재작성, U-120 의존성 갱신), 상세도 유지/향상 확인
- 필수 섹션 누락 없음, 기존 문서 대비 정보량 향상 (U-138: 공개 레포 URL 확정, 문서 영문화 단계 신규, origin 전환 통합)

### 주의사항

- U-138과 U-120의 의존성 순서가 변경됨: **반드시 U-138(공개 레포 준비) 완료 후 U-120(배포) 진행**
- 공개 저장소 URL: `https://github.com/Yachaboom/unknown-world-public.git` (빈 레포, 이미 생성됨)
- `.cursor/`, `.gemini/`, `.claude/` 삭제 여부는 U-138 Q3 페어링 질문으로 남음 (기본: 삭제 권장)
...
**MMP M5에 U-138[Mmp] 신규 추가**: 현재 비공개 저장소의 소스코드를 **공개 GitHub 저장소로 이전**하고, 비밀정보·불필요한 파일(개발 내부 산출물, 테스트 스크린샷, 로컬 유틸 스크립트)을 정리하는 유닛. Devpost 제출 필수 요건인 "Public Code Repo"를 충족하기 위한 작업.

### 영향받은 문서

- 🆕 `vibe/unit-plans/U-138[Mmp].md`: 공개 저장소 이전 + 불필요 파일 정리 계획서
- ✏️ `vibe/roadmap.md`: M5 백로그에 U-138 추가(5→6개), U-121 의존성에 U-138 추가, CP-SUB-01 의존성에 U-138 추가, Devpost 매핑 갱신, 진행률 재계산, 빠른 실행 경로 갱신
- ✏️ `vibe/changelog.md`: 본 이력 추가
...
- U-138[Mmp]: 공개 저장소 이전 + 불필요 파일 정리 (비밀감사/히스토리정리/코드정리) — Devpost "Public Code Repo" 요건 충족

### 의존성 변경

- U-138[Mmp]: Depends=None (U-119/U-120과 병렬 가능)
- U-121[Mmp]: Depends 변경 (이전: U-120 → 이후: U-120,U-138) — README를 공개 저장소에 커밋해야 하므로
- CP-SUB-01: Depends 변경 (이전: U-119,U-120,U-121,U-122 → 이후: U-119,U-120,U-121,U-122,U-138) — 공개 저장소가 제출 필수 항목

### 진행률 변화

- **MVP**: 96.3% (133/138) → 96.3% (133/138) — 변경 없음
- **MMP**: 0% (0/5) → 0% (0/6) — 새 작업 1개 추가
- **전체**: 93.0% (133/143) → 92.4% (133/144) — 분모 +1

### 품질 검증 결과

- **품질 기준 문서**: U-120[Mmp], U-121[Mmp]
- **신규 유닛 계획서**: 1개(U-138) — 필수 섹션 모두 포함 ✅, 삭제/유지 판단 테이블 추가로 상세도 향상
- **로드맵 수정**: M5 백로그, Devpost 매핑, 빠른 실행 경로 갱신 ✅

### 리스크 변경

- 기존 R-025(배포 환경 다운)에 공개 저장소 가용성도 포함하여 대응 범위 확대
- 신규 리스크 없음 (U-138의 잠재적 리스크는 유닛 계획서 내에서 관리)

### 주의사항

- U-138은 U-119(폴리시), U-120(배포)과 **병렬 실행 가능** — 제출 마감 전 동시 진행 권장
- U-121(문서)은 U-138 완료 후 진행해야 공개 저장소에 README를 커밋할 수 있음
- Git 히스토리에 비밀정보 포함 여부에 따라 작업량이 크게 달라질 수 있음 (감사 결과 → 히스토리 정리 여부 결정)
- `vibe/unit-plans/` 유지 여부는 페어링 질문(Q2)으로 결정 보류 — 심사자 관점에서 "개발 방법론 시연" vs "탐색 편의" 트레이드오프

---

## 2026-02-10 - update/U-126-debt-integration (기술 부채 통합)

### 변경 요약

**U-126[Mvp] 유닛 계획서 확장**: `vibe/debt-log.md`에 기록된 미해결 기술 부채 12건 중 MVP 단계에서 해결 가능한 **8건을 U-126에 통합**하고, 대규모 작업이 필요한 **3건은 MMP 이후로 보류**. 기존 성능/품질 최적화(번들/에셋/CSS/렌더링) 범위에 테스트 mock 수정, 기대값 동기화, 스키마 정합, 아키텍처 변경 미반영 테스트 격리를 추가하여 "Phase A: 기술 부채 해소(~30분) + Phase B: 성능 최적화(~60분)" 2단계 구조로 재편.

### 영향받은 문서

- ✏️ `vibe/unit-plans/U-126[Mvp].md`: 기술 부채 8건 통합, 구현 흐름 Phase A/B 분리, 예상 소요 60분→90분, 부채 매핑 테이블 추가, MMP 보류 부채 섹션 추가
- ✏️ `vibe/roadmap.md`: U-126 백로그 설명 변경, 핵심 기능 잔여 항목 갱신(완료된 U-123/U-124/U-125/U-134 제거), 빠른 실행 경로 갱신, 메모 추가
- ✏️ `vibe/changelog.md`: 본 이력 추가
...
- U-126[Mvp]: "MVP 성능/품질 기본 최적화" → "MVP 성능/품질 최적화 + 기술 부채 해소" — debt-log.md 미해결 부채 8건 통합, 예상 소요 60분→90분

### U-126에 통합된 기술 부채 (8건)

| # | debt-log 이슈 (날짜)                                  | 구현 단계 | 수정 대상                                      |
|---|-------------------------------------------------------|-----------|------------------------------------------------|
| 1 | window.matchMedia mock 누락 (2026-02-03)              | A1        | `frontend/src/test/setup.ts`                   |
| 2 | initReactI18next mock 누락 (2026-02-08)               | A2        | `InventoryPanel.test.tsx`                      |
| 3 | previous_image_url 타입 에러 (2026-02-05)             | A2        | `turnStream.*.test.ts`, `i18n-scenario.test.ts`|
| 4 | iconStatus 기대값 불일치 (2026-02-08)                 | A2        | `inventoryStore.test.ts`                       |
| 5 | test_u069_model_tiering 기대값 (2026-02-09)           | A3        | `test_u069_model_tiering.py`                   |
| 6 | 자산 매니페스트 'scene' 누락 (2026-02-09)             | A3        | `test_asset_manifest.py` / 스키마              |
| 7 | render_stage/이미지 생성 테스트 실패 (2026-02-09)     | A4        | `test_u053_*.py`, `test_u054_*.py` (skip 격리) |
| 8 | test_schema_ok flaky (2026-02-10)                     | A4        | `test_real_mode_gate.py` (skip/flaky 격리)     |

### MMP 보류 기술 부채 (3건)

| # | debt-log 이슈 (날짜)                                  | 보류 근거                                          |
|---|-------------------------------------------------------|---------------------------------------------------|
| 1 | backend/tests Pyright 326개 (2026-01-28)              | 프로덕션 코드 0 에러 확보, 대규모 작업(~4h 이상)   |
| 2 | test_real_generator_rembg_integration (2026-02-03)    | CI/CD 환경 인증 이슈, 기능 동작과 무관             |
| 3 | U-066 타자기 효과 속도 불완전 (2026-02-05)            | 현재 속도로 데모 가능, 미세 조정                   |

### 진행률 변화

- **MVP**: 96.3% (133/138) → 96.3% (133/138) — 변경 없음 (유닛 추가/삭제 없이 기존 유닛 범위 확장)
- **MMP**: 0% → 0% (변경 없음)
- **전체**: 93.0% (133/143) → 93.0% (133/143)

### 품질 검증 결과

- **품질 기준 문서**: U-137[Mvp], U-136[Mvp]
- **수정된 유닛 계획서**: 1개(U-126) — 필수 섹션 모두 포함 ✅, 부채 매핑 테이블 + MMP 보류 섹션 추가로 상세도 향상
- **로드맵 수정**: 핵심 기능 잔여 항목 정리, 빠른 실행 경로 현행화 ✅

### 리스크 변경

- 신규/해소 없음 (기존 리스크 유지)
- U-126 범위 확장으로 R-001(스키마 불일치) 대응 강화 (테스트 정합성 확보)

### 주의사항

- U-126 예상 소요가 60분→90분으로 증가. 해커톤 마감 고려 시 Phase A(부채 해소, ~30분)를 우선 수행하고, Phase B(성능 최적화)는 데모 영향도에 따라 범위 조절 가능
- `pytest.mark.skip`으로 격리된 테스트(render_stage, flaky)는 MMP에서 재작성 필요 — debt-log.md에 추적 기록 유지
- 진행률 변화 없음: 신규 유닛 추가가 아닌 기존 U-126의 범위 확장이므로 분모 불변
...
**MVP 유닛 1건 신규 추가**: Panel Corner 이미지 CSS 회전 정합 + 필요 시 방향별 이미지 신규 생성(U-134). `panel-corner-br.png`(BR 방향)를 CSS `rotate()`로 4방향에 재사용하는 과정에서 회전값이 잘못되거나 누락된 곳이 다수 발견됨. M6 skip으로 제거된 U-112[Mmp]의 MVP 승격 대체 유닛.

### 영향받은 문서

- ➕ vibe/unit-plans/U-134[Mvp].md: 신규 — Panel Corner 이미지 방향 수정 (CSS 회전 정합 + nanobanana-mcp 조건부 생성)
- ✏️ vibe/roadmap.md: 백로그에 U-134 추가, 진행률 재계산(128/140), M2 카운트 갱신(11/30), 메모/스탠드업 갱신
...
| ID | 제목 | 의존성 | 우선순위 |
|----|------|--------|----------|
| U-134[Mvp] | Panel Corner 이미지 방향 수정 — CSS 회전 정합 + 필요 시 이미지 신규 생성 | U-032 | Medium (데모 UI 폴리시) |

### 의존성 변경

- U-134: 신규. Depends=U-032(UI Chrome Pack)

### 진행률 변화

- 전체: 128/139 (92.1%) → 128/140 (91.4%)
- MVP: 128/134 (95.5%) → 128/135 (94.8%)
- MMP: 변동 없음 (0/5, 0%)

### 품질 검증 결과

- 품질 기준 문서: U-130[Mvp]
- 신규 유닛 계획서: 1개 생성, 품질 체크리스트 통과
- 수정된 유닛 계획서: 0개

### 리스크 변경

- 없음 (기존 R-003 "채팅 앱 오해" 리스크로 커버 — 코너 방향 어색함은 게임 HUD 인상을 약화시키는 간접 요인)

### 주의사항

- U-134는 M6 skip으로 제거된 U-112[Mmp]의 MVP 승격 대체 유닛이다. U-112 계획서는 참조용으로 유지.
- CSS 회전만으로 해결되면 15분 내 완료 가능. 비대칭 디테일로 방향별 이미지 생성이 필요하면 30분으로 확대.
- nanobanana-mcp로 이미지 생성 시 기존 `panel-corner-br.png`와 동일한 CRT 톤(녹색 글로우, 투명 배경)을 유지해야 한다.

---

## 2026-02-09 - add/story-coherence-lang-impression (U-131~U-133)

### 변경 요약

**MVP 유닛 3건 신규 추가**: 추상적 최종 목표(Overarching Mystery) 시스템(U-131), 영어(en-US) 기본 언어 전환(U-132), 프로필 시작 이미지-스토리 정합성 강화(U-133). 스토리 일관성, Devpost 제출 요건 대응, 데모 첫인상 개선을 위한 보강 유닛들.

### 영향받은 문서

- ➕ vibe/unit-plans/U-131[Mvp].md: 신규 — 추상적 최종 목표(Overarching Mystery) 시스템으로 Quest/스토리 방향성 정렬
- ➕ vibe/unit-plans/U-132[Mvp].md: 신규 — 영어(en-US) 기본 언어 전환 (Devpost 제출 요건 대응)
- ➕ vibe/unit-plans/U-133[Mvp].md: 신규 — 프로필 시작 이미지-스토리 정합성 강화 (첫 턴 맥락 주입)
- ✏️ vibe/roadmap.md: 백로그에 U-131~U-133 추가, 진행률 재계산(11/29), 메모/리스크/스탠드업 갱신
- ✏️ vibe/prd.md: §3.1 기본 언어 영어 정책, §6.1 Overarching Mystery/이미지-스토리 정합성, §데모프로필 이미지-스토리 정합성 추가
- ✏️ vibe/changelog.md: 본 변경 이력 추가
...
| ID | 제목 | 의존성 | 우선순위 |
|----|------|--------|----------|
| U-131[Mvp] | 추상적 최종 목표(Overarching Mystery) 시스템 — Quest/스토리 방향성 정렬 | U-078 | 높음 (스토리 일관성/데모 체감 핵심) |
| U-132[Mvp] | 영어(en-US) 기본 언어 전환 | U-099 | 높음 (Devpost 제출 요건 필수) |
| U-133[Mvp] | 프로필 시작 이미지-스토리 정합성 강화 — 첫 턴 맥락 주입 | U-124, U-131 | 높음 (데모 첫인상/스토리 몰입) |

### 의존성 변경

- U-131: 신규. Depends=U-078(Quest 표시 시스템)
- U-132: 신규. Depends=U-099(영어 모드 품질 검증)
- U-133: 신규. Depends=U-124(사전 생성 이미지), U-131(Overarching Mystery)

### 진행률 변화

- 전체: 119/136 (87.5%) → 119/139 (85.6%)
- MVP: 119/131 (90.8%) → 119/134 (88.8%)

### 리스크 추가

| ID | 리스크 | 완화책 |
|----|--------|--------|
| R-027 | Overarching Mystery가 너무 구체적이면 스토리를 오히려 제약 | 추상성 원칙 명시 + few-shot 템플릿으로 가이드 |
| R-028 | 씬 설명과 실제 이미지 비주얼 불일치 | 씬 설명을 이미지 생성 프롬프트에서 파생하여 일관성 확보 |

---

## 2026-02-08 - add/context-ux-error-boost (U-127~U-130)

### 변경 요약

**MVP 유닛 4건 신규 추가**: 멀티턴 대화 맥락 유지+모델 전환(U-127), 정밀분석 중복 방지(U-128), 아이템 판매 직관적 UX(U-129), 429 Rate Limit 에러 재시도 안내(U-130). 게임 내러티브 품질, 비용 절감, 사용자 경험 안정성을 강화하기 위한 보강 유닛들.

### 영향받은 문서

- ➕ vibe/unit-plans/U-127[Mvp].md: 신규 — 멀티턴 대화 히스토리 맥락 유지 + 기본 텍스트 모델 gemini-3-pro-preview 전환
- ➕ vibe/unit-plans/U-128[Mvp].md: 신규 — 정밀분석 완료 상태에서 정밀분석 카드 비활성화
- ➕ vibe/unit-plans/U-129[Mvp].md: 신규 — 아이템 판매 직관적 UX 개선 (항상 노출 + 확인 절차)
- ➕ vibe/unit-plans/U-130[Mvp].md: 신규 — 429 Rate Limit 에러 시 프론트엔드 재시도 안내 UI
- ✏️ vibe/roadmap.md: 백로그에 U-127~U-130 추가, 진행률 재계산, 메모 추가
- ✏️ vibe/prd.md: §6.1 멀티턴 맥락 유지, §6.2 정밀분석 중복 방지, §6.6 아이템 판매 UX, §8.4 Rate Limit 안내 정책 추가
- ✏️ vibe/changelog.md: 본 변경 이력 추가
...
| ID | 제목 | 의존성 | 우선순위 |
|----|------|--------|----------|
| U-127[Mvp] | 멀티턴 대화 히스토리 맥락 유지 + 기본 모델 gemini-3-pro-preview 전환 | U-069, U-017 | 높음 (내러티브 품질 핵심) |
| U-128[Mvp] | 정밀분석 완료 상태에서 정밀분석 카드 비활성화 | U-090, U-087 | 중간 (비용 절감) |
| U-129[Mvp] | 아이템 판매 직관적 UX 개선 | U-096, U-088 | 중간 (게임 경제 UX) |
| U-130[Mvp] | 429 Rate Limit 에러 시 프론트엔드 재시도 안내 UI | U-087 | 높음 (안정성/UX) |

### 의존성 변경

- U-127: 신규. Depends=U-069(Vertex AI 연동), U-017(Pydantic 스키마)
- U-128: 신규. Depends=U-090(정밀분석 구현), U-087(처리중 입력 잠금)
- U-129: 신규. Depends=U-096(아이템 소비 로직), U-088(인벤토리 DnD)
- U-130: 신규. Depends=U-087(처리중 입력 잠금)

### 진행률 변화

- 전체: 119/132 (90.2%) → 119/136 (87.5%)
- MVP: 119/127 (93.7%) → 119/131 (90.8%)
- MMP: 변동 없음 (0/5, 0%)

### 리스크/참고

- U-127(멀티턴)은 프롬프트 구조와 모델 호출 방식을 근본적으로 변경하므로, 기존 단일턴 동작의 회귀 테스트가 중요함.
- U-127에서 모델 ID 변경(gemini-3-pro-preview)은 tech-stack.md의 모델 라인업과 정합성을 유지해야 함.
- U-130의 `RATE_LIMITED` 에러 코드는 Turn Stream Protocol v1에 추가되는 사실상의 확장이므로, 프로토콜 문서도 갱신 필요.
...
**MVP 유닛 4개 추가 + MMP M6 전체 skip(16개)**: Agent Console 접기 제거 및 배치 재조정(U-123), 프로필별 첫 씬 이미지 사전 생성(U-124), 이전턴 텍스트 주목성 제거(U-125), MVP 성능/품질 기본 최적화(U-126)를 추가하고, 해커톤 일정상 실행 불가능한 M6(품질 강화/후속) 16개 유닛을 전체 skip 처리했다. M6의 핵심 가치(성능/품질)는 U-126으로 압축 대체한다.

### 영향받은 문서

- ✏️ vibe/roadmap.md: MVP 백로그 4개 추가, M6 전체 skip, 진행률 재계산, 마일스톤 갱신, 리스크 R-024 대응 업데이트, 스탠드업 갱신
- 🆕 vibe/unit-plans/U-123[Mvp].md: Agent Console 접기 제거 + 대기열 상단/배지 하단 재배치
- 🆕 vibe/unit-plans/U-124[Mvp].md: 프로필별 첫 씬 이미지 사전 생성 (nanobanana-mcp, 픽셀아트)
- 🆕 vibe/unit-plans/U-125[Mvp].md: 이전턴 텍스트 주목성 제거 - 색상 dim + 폰트 축소
- 🆕 vibe/unit-plans/U-126[Mvp].md: MVP 성능/품질 기본 최적화 (M6 대체, 번들/에셋/CSS/렌더링)

### 백로그 변경

**추가**:

- U-123[Mvp]: Agent Console 접기 제거 + 대기열 상단/배지 하단 재배치 - U-114의 접기 기능을 제거하고, 대기열과 배지를 모두 항상 노출하되 시각적 우선순위 부여
- U-124[Mvp]: 프로필별 첫 씬 이미지 사전 생성 - nanobanana-mcp로 3종 프로필의 픽셀아트 첫 장면 생성, 프로필 시작 즉시 Scene Canvas 표시
- U-125[Mvp]: 이전턴 텍스트 주목성 제거 - 과거 턴 색상 dim + 폰트 축소로 현재 턴에 시선 집중
- U-126[Mvp]: MVP 성능/품질 기본 최적화 - 번들 분석, 미사용 코드/에셋 제거, CSS 정리, React 렌더링 최적화
...
- U-102[Mmp]: GCS 스토리지 어댑터 - **skip** (M6 전체 제거)
- U-103[Mmp]: 이미지 편집(멀티턴) - **skip** (M6 전체 제거)
- U-109[Mmp]: Agentic Vision 자동 실행 - **skip** (M6 전체 제거)
- U-104[Mmp]: 장기 세션 메모리 - **skip** (M6 전체 제거)
- U-105[Mmp]: Scenario Library + 리플레이 - **skip** (M6 전체 제거)
- RU-007[Mmp]: artifacts 정리 - **skip** (M6 전체 제거)
- RU-010[Mmp]: 스키마/상수 SSOT 강화 - **skip** (M6 전체 제거)
- CP-MMP-01: 배포/관측 게이트 - **skip** (M6 전체 제거)
- U-106[Mmp]: 관측 지표/대시보드 - **skip** (M6 전체 제거)
- U-107[Mmp]: 접근성/모바일 UX - **skip** (M6 전체 제거)
- U-108[Mmp]: 보안 하드닝 - **skip** (M6 전체 제거)
- U-110~112[Mmp]: 디버깅/TTL/Panel Corner - **skip** (M6 전체 제거)
- U-113[Mmp]: 세션 상태 영속성 - **skip** (M6 전체 제거)
- CP-MMP-02: 시나리오 회귀 100% - **skip** (M6 전체 제거)

### 의존성 변경

- U-123[Mvp]: 신규 Depends=U-114
- U-124[Mvp]: 신규 Depends=U-084,U-116
- U-125[Mvp]: 신규 Depends=U-086
- U-126[Mvp]: 신규 Depends=None
- R-024 대응: ~~U-113[Mmp]~~ → 해커톤 이후 재계획

### 진행률 변화

- **MVP**: 119/124 (96.0%) → 119/128 (93.0%) (신규 4개 추가로 분모 증가)
- **MMP**: 0/21 (0%) → 0/5 (0%) (M6 16개 skip으로 분모 감소)
- **전체**: 119/145 (82.1%) → 119/133 (89.5%) (net -12 = 신규 +4, skip -16)

### 품질 검증 결과

- **품질 기준 문서**: U-117[Mvp] (인벤토리 드래그 확장), U-116[Mvp] (SaveGame 제거)
- **신규 유닛 계획서**: 4개(U-123~U-126) 생성
- **품질 체크**: 모든 필수 섹션(메타데이터/목표/파일/흐름/의존성/주의사항/질문/참고) 포함 ✅
- **완료 기준**: 각 유닛 3개 이상 ✅
- **구현 단계**: 각 유닛 4~6단계 ✅
- **M6 skip**: 16개 유닛 모두 skip 처리, 대체 유닛(U-126) 명시 ✅

### 리스크 변경

**수정**:

- R-024: 대응을 ~~U-113[Mmp]~~ → "해커톤 이후 재계획"으로 변경 (M6 skip으로 U-113 제거)

**해소(부분)**:

- M6 제거로 로드맵 복잡도 대폭 감소, 해커톤 집중도 향상

### 주의사항

- **M6 skip은 "영구 삭제"가 아닌 "해커톤 이후 재계획"**: 보안 하드닝(U-108), 세션 영속성(U-113) 등 중요한 유닛은 해커톤 이후 별도로 재계획해야 함.
- **U-123(Agent Console 재배치)**: U-114의 접기 기능을 제거하므로, U-114 결과물을 먼저 확인 후 수정.
- **U-124(프로필 씬 이미지)**: U-084(픽셀 스타일)가 완료된 후 동일 스타일로 생성해야 일관성 유지.
- **U-125(이전턴 약화)**: opacity + dim이 과도하면 가독성 저하 → 보수적 값으로 시작.
- **U-126(성능 최적화)**: 코드 삭제 시 참조 검색 철저히 수행, 기능 회귀 방지.
