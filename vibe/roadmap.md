...
**전체**: 7/50 (14%) | **MVP**: 7/37 (19%) | **MMP**: 0/13 (0%)
...
| 단계 | ID        | 이름                                   | 목표일     | 진행률 | 상태 |
| ---- | --------- | -------------------------------------- | ---------- | ------ | ---- |
| MVP  | M1        | 스캐폴딩 + Turn 계약 + HTTP Streaming  | 2026-01-05 | 7/11   | 🚧   |
...
### MVP
ID=[U-007[Mvp]](unit-plans/U-007[Mvp].md) | ⚡모의 Orchestrator + /api/turn HTTP Streaming(POST) | Depends=U-005 | 🚧
...
# 전체 포트 정리 (8001~8020)
pnpm kill:port
# ⚠️ pnpm kill 은 현재 node.exe 전체 종료(다른 프로젝트 영향) → U-027에서 포트 기반으로 제한 예정. 지금은 pnpm kill:port 사용 권장.
```
