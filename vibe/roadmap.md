...
**전체**: 1/49 (2%) | **MVP**: 1/36 (2%) | **MMP**: 0/13 (0%)

**예상 완료(가정)**: MVP D-13 | MMP D-29  
_가정: 1인 기준 / 1일 순개발 4h / 유닛 평균 45분 / 버퍼 30% 포함_

**진행 중**: [U-002[Mvp]](unit-plans/U-002[Mvp].md) / **최근 완료**: [U-001[Mvp]](unit-plans/U-001[Mvp].md) (2026-01-03)
...
| R-002 | 지연/비용(특히 이미지/Thinking)으로 데모 체감 저하(TTFB>2s, 비용 폭증) | High | 30%  | SSE 단계/배지 우선 스트리밍 + 경제(예상비용/대안) + Lazy 이미지 + 정책 프리셋         |
...
| 단계 | ID        | 이름                                   | 목표일     | 진행률 | 상태 |
| ---- | --------- | -------------------------------------- | ---------- | ------ | ---- |
| MVP  | M1        | 스캐폴딩 + Turn 계약 + SSE 스트리밍    | 2026-01-05 | 0/10   | ⏸️   |
...
- **완료 기준**: SSE로 Queue/Badges/Auto-repair를 스트리밍하고, 최종 TurnOutput이 스키마/비즈니스 룰을 통과
...
ID=[U-002[Mvp]](unit-plans/U-002[Mvp].md) | ⚡프론트 Vite+React+TS 초기화 (/frontend) | Depends=U-001 | ⏸️
ID=[U-003[Mvp]](unit-plans/U-003[Mvp].md) | ⚡백엔드 FastAPI 초기화 (/backend) | Depends=U-001 | ⏸️
ID=[U-004[Mvp]](unit-plans/U-004[Mvp].md) | ⚡CRT 테마/고정 레이아웃 스켈레톤 (/frontend/src) | Depends=U-002 | ⏸️
ID=[RU-001[Mvp]](unit-plans/RU-001[Mvp].md) | 리팩토링: 디렉토리/설정 정리 | Depends=U-004,U-003 | ⏸️
...
ID=[U-024[Mvp]](unit-plans/U-024[Mvp].md) | ⚡Backend Autopilot(제한 스텝) + Action Queue SSE | Depends=U-018,U-023 | ⏸️
...
- [ ] SSE 연결이 되고 Agent Console에 단계/배지가 보인다(프롬프트 원문 노출 없음)
