...
**전체**: 49/73 (67%) | **MVP**: 49/60 (81%) | **MMP**: 0/13 (0%)
...
**진행 중(현재 포커스)**: [U-043[Mvp]](unit-plans/U-043[Mvp].md) / **최근 완료**: [U-036[Mvp]](unit-results/U-036[Mvp].md) (2026-01-26)
...
- **완료 기준**: 텍스트 우선 + (조건부) 이미지 생성/표시, Scanner 업로드가 "아이템/단서"로 변환되어 인벤토리에 반영, 오브젝트 이미지 배경 제거(rembg) 지원, 프롬프트 파일 분리/핫리로드 지원
...
ID=[U-044[Mvp]](unit-plans/U-044[Mvp].md) | 세션 언어 SSOT(토글=리셋) + 혼합 출력(상태/시스템) 제거 | Depends=U-015,U-039,U-043 | 🚧
