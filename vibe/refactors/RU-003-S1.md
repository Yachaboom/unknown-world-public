...
- `executeTurnStream`은 AbortError인 경우 `onComplete`를 호출하지 않는 정책입니다(`turnStream.ts` 375~402).  
  현재 `frontend/src/turn/turnRunner.ts`에 `cancel()` 기본 골격이 추가되어(Abort만 수행), Cancel 버튼이 이 경로를 호출하면 **isStreaming이 해제되지 않아 UI가 영구 로딩/무반응처럼 보이는** 유형의 UX 장애로 이어질 수 있습니다.
...
- `frontend/src/turn/turnRunner.ts`
  - `cancel()` 기본 골격(Abort 전파) 및 취소 UX 확장 포인트
...
- **현 구현 상태(메모)**: `frontend/src/turn/turnRunner.ts`의 `cancel()`은 Abort만 수행하는 기본 골격입니다. 현 정책(Option B)에서는 Abort가 `onComplete`를 트리거하지 않으므로, Cancel UX는 “기본 골격만 구현” 상태로 간주해야 합니다.
...
- `frontend/src/turn/turnRunner.ts` (Cancel/Abort 골격, 콜백 라우팅)
