...
- **Dev/Build Tools (에셋 제작 전용)**
  - rembg `2.0.67` (배경 제거, **개발 시점에만 사용**)
    - 개발용: nanobanana mcp 에셋 투명 PNG 제작 (로컬 도구)
    - (제거) 런타임 실시간 배경 제거 제거 (부팅 속도 최적화)
    - 가이드: `vibe/ref/rembg-guide.md` 기준
...
| 오브젝트 이미지 배경 제거      | rembg (**개발 시점 전처리**)    | 투명 합성 지원      |
...
- **1) 제품/UX 이해**: PRD 6~9장(게임 UI·경제·에이전트 가시화) 훑고 “채팅 UI 금지” 원칙 공유
