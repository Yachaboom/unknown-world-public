...
심사자/사용자가 10분 안에 핵심 기능을 모두 체험할 수 있도록, “즉시 시작 → 조작(클릭/드래그/업로드) → 룰 변형 → 엔딩 리포트 → 리셋”의 **데모 루프**를 수동 검증하고 런북/증거를 남긴다.
...
- **계획서**: [U-025[Mvp]](U-025[Mvp].md) - 엔딩 리포트 + 리플레이 하네스
...
- [x] **Q1**: CP 결과물의 “증거”는 어느 수준까지 남길까
  - ✅ Option A: 스크린샷 + 체크리스트 + 로그 요약(권장)
  - Option B: 10분 데모 영상(신뢰도↑, 제작 부담↑)

## 참고 자료

- `vibe/prd.md` - Demo Mode/엔딩 리포트
