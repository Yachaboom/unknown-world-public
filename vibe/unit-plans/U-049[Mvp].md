...
초기 레이아웃에서 불필요한 스크롤/중첩 스크롤이 발생해 “한눈에 보이지 않는” 문제를 줄이고, 특히 **재화(Economy) 패널은 카드 내부(원장 리스트)만 스크롤**되도록 스크롤 전략을 정리한다.
...
- Economy HUD는 잔액/비용 영역은 항상 보이고, 긴 원장(ledger) 목록만 내부 스크롤로 처리된다(스크롤 포커스가 우측 컬럼 전체로 튀지 않음).
...
- `frontend/src/components/EconomyHud.tsx` - 원장(ledger) 렌더 구조
...
- [x] **Q1**: 우측 사이드바의 “유연 높이(flex-1)”는 어느 패널이 가져갈까?
  ...
  - Option B: Economy도 `flex: 1`로 확장(원장 많이 볼 때 유리, 대신 콘솔이 줄어듦)
  **A1**: Option A
