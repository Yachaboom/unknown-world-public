...
인벤토리 아이템이 많아질 경우에도 **패널 레이아웃이 깨지지 않고 스크롤로 모든 아이템에 접근**할 수 있게 한다. 또한 아이템 개수 표시, 정렬/필터(선택), 빈 슬롯 상태 등 **아이템 관리 UX**를 개선한다. 동시에, Quest/Rule 패널이 확장되어도 **Inventory 패널의 최소 가시성을 보장**하는 사이드바 패널 영역 분배를 함께 처리한다.

**배경**: 현재 인벤토리 패널(U-011)은 기본 DnD 기능만 구현되어 있어, 아이템이 5~10개를 넘어가면 패널 영역을 넘치거나 레이아웃이 깨질 수 있다. 또한 Quest/Rule 패널의 항목이 늘어나면 Inventory 패널이 축소/가려져 핵심 인터랙션(아이템 DnD)이 불가능해지는 문제도 존재한다. (기존 U-081 범위를 본 유닛에 흡수)
...
- **(U-081 흡수)** Quest/Rule 패널에 **max-height + 내부 스크롤** 적용, Inventory 패널에 **min-height 보장** (flex-shrink: 0)
- **(U-081 흡수)** 패널 간 영역 분배가 안정적으로 유지되어, 모든 핵심 패널 헤더가 항상 보임

## 영향받는 파일

**수정**:

- `frontend/src/components/InventoryPanel.tsx` - 스크롤 컨테이너, 아이템 개수 표시, 빈 상태 UI, min-height 적용
- `frontend/src/components/QuestPanel.tsx` - max-height + 내부 스크롤 적용 (U-081 흡수)
- `frontend/src/components/RuleBoard.tsx` - max-height + 내부 스크롤 적용 (U-081 흡수)
- `frontend/src/components/Sidebar.tsx` (또는 해당 레이아웃 컴포넌트) - 패널 flex 배치 및 영역 분배 (U-081 흡수)
- `frontend/src/style.css` - 인벤토리 스크롤 스타일, 사이드바 패널 레이아웃 CSS 조정
...
### 4단계: (U-081 흡수) 사이드바 패널 영역 분배

- Inventory에 `min-height: 120px`, `flex-shrink: 0` 적용 (절대 축소 금지)
- Quest/Rule 패널에 `max-height: 200px`, `overflow-y: auto` 적용
- 각 패널 헤더가 스크롤 영역 밖에 위치하여 항상 보이도록 구조 조정
- 뷰포트 높이가 작을 때(768px 이하) 반응형 미디어 쿼리로 min-height/max-height 조정
...
- U-082: Agent Console 영역 조정과 함께 전체 레이아웃 균형 확보 (기존 U-081→U-082 전달 경로 흡수)
