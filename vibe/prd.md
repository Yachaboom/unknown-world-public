...
- **(보강, MVP) 추상적 최종 목표(Overarching Mystery)**: 모든 세션에 공통된 **추상적이고 미스터리한 최종 지향점(Overarching Mystery)**을 시스템 프롬프트에 고정한다. 이 목표는 특정 장르/스토리에 종속되지 않는 범용적 메타포이며, GM이 매 턴 내러티브와 퀘스트를 생성할 때 **방향성 맥락**으로 참조하여 Quest와 스토리의 정합성을 유지한다. ⏸️ (U-131)
