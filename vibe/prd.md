...
- **(보강, MVP) 멀티턴 대화 맥락 유지**: GM은 이전 턴들의 대화 히스토리(user/model contents)를 유지하여, 스토리·NPC·환경 맥락이 턴 간에 일관되게 이어지도록 한다. Gemini 3 Thought Signature를 활용해 추론 맥락을 보존하고, 토큰 예산 내에서 슬라이딩 윈도우로 히스토리를 관리한다. ⏸️ (U-127)
...
- **(MVP 정책) 핫스팟 생성 제한**: 핫스팟(ui.objects[])은 **"정밀분석" 액션을 통해서만 생성**한다. 일반 턴에서 GM이 핫스팟을 임의 생성하는 것을 금지하여, 이미지-핫스팟 정합성을 보장한다. ✅ (U-090)
  ...
  - **(MVP 보강) 정밀분석 중복 방지**: 정밀분석으로 핫스팟(sceneObjects)이 이미 화면에 존재하는 상태에서는 정밀분석 카드를 **비활성화**하여, 불필요한 중복 API 호출과 비용 소모를 방지한다. 새 씬 이미지가 생성되면 자동 재활성화된다. ⏸️ (U-128)
...
- **Inventory(인벤토리) + Drag & Drop(드래그 사용)**:
  ...
  - **(MVP 보강) 아이템 판매 직관적 UX**: 아이템 판매 버튼을 **항상 노출**하여 재화 잔액과 무관하게 판매할 수 있게 한다. 판매 가격(Signal)을 카드에 명시하고, 오조작 방지를 위한 **인라인 확인 절차**(2초 타이머)를 적용한다. ⏸️ (U-129)
...
- **검증 배지(Proof of System)**:
  ...
  - **(MVP 정책 변경)** 검증 배지와 대기열(Action Queue) **모두 항상 노출**한다. 대기열을 상단에, 배지를 하단에 배치하여 "에이전트 시스템"의 단계별 진행과 검증 결과를 동시에 보여준다. ~~접기 토글은 제거한다.~~ ✅ (U-114) → ⏸️ (U-123: 접기 제거 + 재배치)
...
- **검증/복구**:
  ...
  - **(보강, MVP) Rate Limit(429) 사용자 안내**: 백엔드에서 기본 모델과 폴백 모델 모두 429 에러로 실패한 경우, 프론트엔드에 `RATE_LIMITED` 에러 코드를 전달하고, 사용자에게 **1~2분 후 재시도**를 안내하는 전용 UI(CRT 테마 패널 + 재시도 버튼)를 제공한다. ⏸️ (U-130)
