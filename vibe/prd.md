...
- **(MVP 정책) 핫스팟 생성 제한**: 핫스팟(ui.objects[])은 **"정밀분석" 액션을 통해서만 생성**한다. 일반 턴에서 GM이 핫스팟을 임의 생성하는 것을 금지하여, 이미지-핫스팟 정합성을 보장한다. ⏸️ (U-090)
...
- **Inventory(인벤토리) + Drag & Drop(드래그 사용)**:
  ...
  - **(MVP 보강)** 아이템 사용 시 **일회용 아이템은 소비(인벤토리에서 삭제)**. 재사용 가능 아이템은 유지. GM이 맥락에 따라 판단. ⏸️ (U-096)
...
- **인터랙션 안내 UX 및 온보딩 가이드**: (✅ 구현 완료)
  - 핫스팟(클릭) 및 인벤토리 아이템(드래그)에 대한 **Contextual Tooltip** 제공.
  - 첫 세션 시작 시 핵심 조작법을 안내하는 **3단계 팝업 온보딩 가이드** 제공.
  - 학습 효과를 고려하여 첫 3회 노출 후 자동으로 숨겨지는 지능형 힌트 시스템 적용. ✅ (U-074)
