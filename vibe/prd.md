...
1. **Intro**: Start → 언어 선택(ko/en) → 기본 재화 지급
...
### 8.2 인증/런타임: Vertex AI 서비스 계정(필수)
...
- **모델 선택**
  - 빠른 생성/빈번한 장면: `gemini-2.5-flash-image`
  - 고품질/텍스트 렌더링/멀티턴 편집: `gemini-3-pro-image-preview`
  - 입력 이미지 참고가 많을수록(다중 참조) `gemini-3-pro-image-preview`를 우선 고려(참조 이미지 수/품질에 유리)
- **프롬프트 원칙**
