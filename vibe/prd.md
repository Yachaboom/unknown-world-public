...
- **검증 배지(Proof of System)**:
  ...
  - **(MVP 정책 변경)** 검증 배지는 **기본적으로 접힌 상태**로 두고, 대기열(Action Queue)이 **항상 노출**되도록 변경한다. 에이전트의 단계별 진행 상황이 실시간으로 보이는 것이 "채팅이 아닌 시스템"을 증명하는 데 더 효과적이다. 실패 배지(fail)는 접힌 상태에서도 경고 아이콘으로 축약 표시한다. ⏸️ (U-114)
