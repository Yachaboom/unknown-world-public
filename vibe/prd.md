...
- (MVP) **기본 언어: 영어(en-US)**: Devpost 해커톤 제출 요건("English language")에 대응하여, 첫 접속 시(LocalStorage에 설정이 없는 경우) **영어로 시작**한다. 한국어는 언어 토글로 전환 가능하다. ⏸️ (U-132)
...
- **(보강, MVP) 멀티턴 대화 맥락 유지**: GM은 이전 턴들의 대화 히스토리(user/model contents)를 유지하여, 스토리·NPC·환경 맥락이 턴 간에 일관되게 이어지도록 한다. Gemini 3 Thought Signature를 활용해 추론 맥락을 보존하고, 토큰 예산 내에서 슬라이딩 윈도우로 히스토리를 관리한다. ✅ (U-127)
- **(보강, MVP) 추상적 최종 목표(Overarching Mystery)**: 모든 세션에 공통된 **추상적이고 미스터리한 최종 지향점(Overarching Mystery)**을 시스템 프롬프트에 고정한다. 이 목표는 특정 장르/스토리에 종속되지 않는 범용적 메타포이며, GM이 매 턴 내러티브와 퀘스트를 생성할 때 **방향성 맥락**으로 참조하여 Quest와 스토리의 정합성을 유지한다. ✅ (U-131)
- **(보강, MVP) 프로필 시작 이미지-스토리 정합성**: 프로필 시작 시 사전 생성 이미지(U-124)의 **씬 설명(scene description)**을 첫 턴 프롬프트에 맥락으로 주입하여, 환영 메시지 → 이미지 → 첫 턴 내러티브가 자연스럽게 이어지도록 한다. 이미지는 "스타일 레퍼런스"가 아닌 **"이야기의 한 축"**으로 기능한다. ⏸️ (U-133)

### 6.2 구조화 UI 자동 생성(클릭 가능한 세계) (✅ 구현 완료)

- 오브젝트/핫스팟을 좌표로 제공하여, 화면에서 클릭 가능하게 한다. ✅
- 좌표는 **0~1000 정규화 좌표계**를 사용(이미지 이해 bbox 포맷과 호환). ✅
- **(MVP 정책) 핫스팟 생성 제한**: 핫스팟(ui.objects[])은 **"정밀분석" 액션을 통해서만 생성**한다. 일반 턴에서 GM이 핫스팟을 임의 생성하는 것을 금지하여, 이미지-핫스팟 정합성을 보장한다. ✅ (U-090)
  - **(MVP 보강) 핫스팟 수량/형태 제한**: 정밀분석을 통한 핫스팟 생성 시, **한 번에 1~3개만 생성**하며, **크기·중요도·적합도** 기준으로 우선순위를 부여하여 선택한다. 겹치는 핫스팟은 우선순위가 낮은 것을 배제한다. 핫스팟은 오브젝트 대상 중앙에 **컴팩트한 원형(circle)**으로 표현하며, 펄스 애니메이션 등으로 식별성을 확보한다. ⏸️ (U-115)
  - **(MVP 보강) 정밀분석 중복 방지**: 정밀분석으로 핫스팟(sceneObjects)이 이미 화면에 존재하는 상태에서는 정밀분석 카드를 **비활성화**하여, 불필요한 중복 API 호출과 비용 소모를 방지한다. 새 씬 이미지가 생성되면 자동 재활성화된다. ✅ (U-128)

### 6.3 멀티모달 렌더링 파이프라인

- 텍스트 상황 묘사 → (조건부) 이미지 생성/편집 → UI 오버레이(핫스팟) 제공 ✅
- 이미지 생성이 느릴 경우 **텍스트 우선 출력 + Lazy Loading**. ✅ (U-031 Placeholder 적용)
- (보강, MVP) 이미지 생성 지연이 길어도 “멈춤/실패”로 느껴지지 않게 **진행 연출 + late-binding**으로 흡수한다:
  - 텍스트/패널/룰/인벤토리는 먼저 갱신되고, Scene Canvas는 “형성 중” 상태(이전 이미지 유지 + 로딩 라벨)로 자연스럽게 유지 ✅ (U-066)
...
- **오브젝트/아이템 이미지 배경 제거(조건부)**: 투명 합성이 필요한 경우 `rembg`를 사용해 배경을 제거한다. (가이드: `vibe/ref/rembg-guide.md`) ⏸️ (에셋 파이프라인 내 구현됨)
  ...
  - **(MVP 정책 변경)** 런타임 rembg 파이프라인은 **일괄 제거** 예정(U-091). 배경 제거가 필요한 에셋은 **개발 시점(Dev-only)**에서 미리 처리하거나, 프리셋 이미지를 사용한다. rembg는 Dev-only 도구로만 유지한다.
...
- **(MMP) 스토리지 TTL/정리 정책**:
  - 생성 이미지/업로드 이미지/아티팩트(엔딩 리포트 등)의 **TTL(Time-To-Live) 및 자동 정리 정책**을 정의한다.
  - 로컬 개발: 세션 종료 또는 일정 시간 후 임시 파일 정리(권장: 24h TTL)
  - 클라우드(GCS 등): 버킷 Lifecycle 정책으로 만료/아카이브/삭제 자동화
  - 사용자 업로드 이미지(Scanner)는 세션 범위 임시 저장 후 정리(민감 데이터 잔류 방지)
...
- **Inventory(인벤토리) + Drag & Drop(드래그 사용)**:
  ...
  - **(MVP 보강)** 아이템 사용 시 **일회용 아이템은 소비(인벤토리에서 삭제)**. 재사용 가능 아이템은 유지. GM이 맥락에 따라 판단. ✅ (U-096)
  - **(MVP 보강)** 아이템 드래그 시작 영역을 **Row 전체**로 확장하여 조작 편의성을 향상한다. 단, **드래그 중 고스트(미리보기) 이미지는 아이콘만** 표시하여 시각적 깔끔함을 유지한다. ⏸️ (U-117)
  - **(MVP 보강) 아이템 판매 직관적 UX**: 아이템 판매 버튼을 **항상 노출**하여 재화 잔액과 무관하게 판매할 수 있게 한다. 판매 가격(Signal)을 카드에 명시하고, 오조작 방지를 위한 **인라인 확인 절차**(2초 타이머)를 적용한다. ✅ (U-129)
...
- **Memory Pin UI(중요 설정 고정)**:
  ...
  - **(멀티모달 데모) Scanner 슬롯(이미지 드랍/업로드)**: (✅ 구현 완료)
  - 사용자가 이미지를 드래그/업로드하면, 이미지 이해(캡션/오브젝트 감지)를 통해 **아이템/단서**로 변환해 인벤토리 추가하거나 세계에 배치
  - 예: "현실 사진 → 게임 속 '부품' 아이템화", "스케치 → 오브젝트 생성"
  - **(비용 정책, MVP)**: Scanner 분석(POST `/api/scan`)은 현재 **Signal을 소모하지 않음**. 다만, 이미지 분석은 모델 호출 비용이 발생하므로 MMP에서 비용 부과 여부를 재검토할 수 있다.
  - **(안정성 정책, MVP)**: 이미지 이해 응답 파싱 실패 시 **최대 2회 자동 재시도 및 지수 백오프**를 적용하여 분석 성공률을 극대화한다. ✅ (U-094)
  - **(아이템 생성 개수 정책, MVP)**: Scanner 스캔 시 **1~3개 아이템을 랜덤으로 생성**한다(가중치: 1개=60%, 2개=30%, 3개=10%). 서버에서 개수를 결정하여 프롬프트에 지시한다. ⏸️ (U-095)
  - **(프롬프트 최적화 계획)**: Real mode(실모델) 환경에서의 비전 모델 프롬프트 튜닝 및 bbox/캡션 품질 개선은 MMP에서 진행한다.
- **인터랙션 안내 UX**: (✅ 구현 완료, 일부 변경)
  - 핫스팟(클릭) 및 인벤토리 아이템(드래그)에 대한 **Contextual Tooltip** 유지.
  - ~~첫 세션 시작 시 핵심 조작법을 안내하는 **3단계 팝업 온보딩 가이드** 제공.~~ **(MVP 정책 변경)** 스캐너 첫 시작 안내 팝업을 제거한다. 게임 플로우를 방해하지 않는 hover 힌트만 유지한다. ⏸️ (U-118)
  - 학습 효과를 고려하여 첫 3회 노출 후 자동으로 숨겨지는 지능형 힌트 시스템 유지. ✅ (U-074)
### 6.8 (데모 표면 핵심) 에이전트 동작 가시화(Action Queue / Self-Repair)
...
- **검증 배지(Proof of System)**:
  ...
  - **(MVP 정책 변경)** 검증 배지와 대기열(Action Queue) **모두 항상 노출**한다. 대기열을 상단에, 배지를 하단에 배치하여 "에이전트 시스템"의 단계별 진행과 검증 결과를 동시에 보여준다. 접기 토글은 제거한다. ✅ (U-123)
...
- **프리셋 구성**: 초기 재화/세이브/튜토리얼 진행 상태가 서로 달라, 10분 안에 “드래그·클릭·스캐너·룰 변형·엔딩 리포트”를 모두 확인 가능
- **사전 생성 이미지 (U-124)**: 프로필 시작 시 발생하는 이미지 생성 지연(10~20초)을 제거하기 위해, 각 프로필의 세계관에 맞는 첫 번째 씬 이미지를 사전 생성하여 즉시 표시한다. ✅ (U-124)
- **(보강, MVP) 이미지-스토리 정합성 (U-133)**: 사전 생성 이미지의 **씬 설명(scene description)**을 첫 턴 프롬프트에 주입하여, 이미지가 "스타일 레퍼런스"에 그치지 않고 **이어지는 스토리의 한 축**으로 동작하게 한다. 환영 메시지 → 이미지 → 첫 턴 내러티브가 하나의 장면으로 자연스럽게 이어짐. ⏸️ (U-133)
...
- **검증/복구**:
  ...
  - **Scanner 통합**: GM 턴뿐만 아니라 Scanner(`POST /api/scan`) 파이프라인에도 자동 재시도 및 보정 프롬프트 기반의 Repair loop를 적용하여 시스템 전반의 복원력을 강화함. ✅ (U-094)
  - **(보강, MVP) Rate Limit(429) 사용자 안내**: 백엔드에서 기본 모델과 폴백 모델 모두 429 에러로 실패한 경우, 프론트엔드에 `RATE_LIMITED` 에러 코드를 전달하고, 사용자에게 **1~2분 후 재시도**를 안내하는 전용 UI(CRT 테마 패널 + 재시도 버튼)를 제공한다. ✅ (U-130)
...
- **모델 선택(고정)**
  ...
  - (선택) 저지연 프리뷰/초안: `gemini-2.5-flash-image` (FAST, 지연 흡수 플로우에서 사용 가능; 계획: U-066)
...
- **(MMP) Agentic Vision(생성된 장면 이미지 기반 ‘행동/핫스팟 후보’ 추출)**:
  ...
  - 적용 정책: Key scene만(예: “장면 생성 직후 1회”), 또는 Assist 모드에서만 실행 등 **비용/지연 제어**를 기본으로 두고, 실패/차단 시 텍스트-only로 폴백한다. (RULE-004/005/008)
- **입력 방식**
...
- **UI 형태 원칙**:
  ...
  - **(MVP 보강) 이전 턴 텍스트 주목성 제거**: 과거 턴 텍스트는 **색상을 어둡게(dim)** + **폰트 축소**하여 현재 턴에 시선 집중. RPG 게임 로그 패턴(최신 강조, 과거 페이드). ⏸️ (U-125)
...
- 스크롤은 “컬럼 전체(사이드바)”가 아니라 **패널 콘텐츠 단위**로 한정한다(중첩 스크롤/전체 패널 스크롤 UX 악화 방지).
  - 예: Economy HUD는 잔액/비용은 항상 보이고, 긴 거래 장부(ledger) 목록만 **카드 내부 스크롤**로 처리한다.
...
- **사이즈/성능 예산(권장)**:
  ...
  - placeholder / Start Scene: 1개 200KB 이하 권장 (U-124 반영)
...
- **Hot controls**:
  - 언어(ko/en), 테마(dark/light)
    - (MVP) 언어 전환은 "리셋 동반"으로만 적용하여 한 화면에 ko/en 혼합이 남지 않게 한다.
  - "이미지 생성 빈도/해상도 정책" 프리셋(예: `Story-only`, `Key Scenes`, `Every Turn`)
- **(MMP) 디버깅 모드 토글 UI**:
  - 프론트엔드에서 "개발자/심사자 모드"를 토글할 수 있는 버튼/패널을 제공한다.
  - 토글 시 추가로 노출되는 정보: 스트림 이벤트 raw 로그(타입/타이밍), 상태 스냅샷 diff, 스토리지 사용량 등
  - 단, 프롬프트 원문/CoT/비밀정보는 절대 노출하지 않는다(메타/요약만). (RULE-007/008)
- **프롬프트 노출 금지**:
